...
Blocked: a macOS 14+ build and test host is required before implementation can begin.
...
- Current blocker: this Windows host does not provide a macOS SDK or Swift toolchain; `project-sources.yml` has `macos: pending`.
- Next action: provide the reachable macOS source/build host path, then perform step 1.